...
loss_list, eval_list = train(net, inputs,targets, loss = MSE() ,optimizer = LM_cond(float(sys.argv[1]),float(sys.argv[2])), iterator = BatchIterator(batch_size =  2), num_epochs = n_epochs, eps = eps)
...
aux = np.arange(6)
plt.scatter(aux,aux,s = 30, c = "red")
plt.plot(ex,ey)
#plt.savefig(f'Figuras/Square/EQ.png', format='png')
plt.show()
...
with open('eval_file_2_lin.csv', mode='w') as res_file:
