...
def calcula_G(hist, k, G, gamma):
    return gamma*G + (1.0 - gamma)*quad(hist[k])
...
def RMSProp(X, y, ni, epsilon,gamma, batch_size = 5.0):
    ...
    while n_iter < 5000:
        ...
        G = calcula_G(g_hist, n_iter, G, gamma)
        G_h = calcula_G(gh_hist, n_iter, G_h, gamma)
...
#=============== plotando resultados ====================
    ...
    plt.title("RMSProp com ni = {}, epsilon = {}, gamma = {} e batch_size = {} \nRegressão de x²: m = {}".format(ni,epsilon, gamma,batch_size, m))
...
RMSProp(X, y, float(sys.argv[1]) , 1e-8, 0.9  ,int(sys.argv[2]))