...
plt.title("Regressão de x² - LM com busca linear")
...
plt.plot(ex,ey, label = f'alpha = {1e3:.1E}\nloss = {loss_list[len(loss_list) - 1]:.02f}')
...
plt.plot(ex,ey, label = f'alpha = {1e4:.1E}\nloss = {loss_list[len(loss_list) - 1]:.02f}')
...
plt.plot(ex,ey, label = f'alpha = {1e5:.1E}\nloss = {loss_list[len(loss_list) - 1]:.02f}')

plt.legend()

plt.savefig(f'Figuras/Square/REG_lmbl.png', format='png')
plt.show() 
